...
<div class="x-nav">
    ...
    <a class="layui-btn layui-btn-sm" style="line-height:1.6em;margin-top:3px;float:right" href="<%=path%>/findChecked" title="刷新">
        <i class="layui-icon" style="line-height:30px">&#xe669;</i></a>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="<%=path%>/findChecked" >
    ...
    <xblock>
        <button id="addCheckedBtn" class="layui-btn layui-btn-sm"> <i class="layui-icon">&#xe654;</i>添加考核 </button>
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${pageList.list}" var="di">
            <tr>
                  ...
                  <th>${di.ptime}</th>
                <td>
                    <a title="编辑" id= "updateEdit" href="<%=path%>/findCheckedById?id=${di.id}">
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addCheckedForm">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="lnum" lay-verify="required" class="layui-input" onKeyUp="this.value=this.value.replace(/[^\.\d]/g,'');this.value=this.value.replace('.','');">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">加班次数：</label>
                    <div class="layui-input-block">
                        <input type="text" name="wnum" lay-verify="required" class="layui-input" onKeyUp="this.value=this.value.replace(/[^\.\d]/g,'');this.value=this.value.replace('.','');">
                    </div>
                </div>
                <div class="layui-form-item">
    ...
    <script>
                ...
                form.on('submit(formDemo)', function(data) {
                    ...
                    $.ajax({
                        ...
                        success:function(){
                            ...
                            setTimeout(function () {window.location.href='<%=path%>/findChecked';},2000);
        ...
        function member_del(obj,id){
            layer.confirm('确认要删除吗？',function(index){
                //发异步删除数据
                $.get("<%=path%>/deleteChecked",{"id":id},function (data) {
                    if(data =true){
